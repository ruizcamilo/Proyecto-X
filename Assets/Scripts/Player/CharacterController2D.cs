...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
        ...
        if (_playerIsSlow || _playerIsHeavy)
            updateCounter();
        ...
        if (isGrounded && !_playerIsSingleJump) canDoubleJump = true;
		...
		if(isGrounded && Input.GetButtonDown("Jump")) // If grounded AND jump button pressed, then allow the player to jump
		{
            ...
            DoJump();
		}else if( !_playerIsSingleJump && canDoubleJump && Input.GetButtonDown("Jump") ) //If can double jump AND jump button pressed And the player is not grounded.
        {
            //Makes the player jump while in mid air and changes "canDoubleJump" to false
            DoJump();
            canDoubleJump = false;
        }
	...
	IEnumerator KillPlayer()
	{
		if (playerCanMove)
		{
            ...
            //Código comentado hasta que sea util
    ...
    public void singleJumpPlayer()
    {
        _playerIsSingleJump = true;

    }

    //Make player single jumper
    IEnumerator singleJumpPlayerRoutine()
    {
        yield return new WaitForSecondsRealtime(debuffTime);
        _playerIsSingleJump = false;
    }
    ...
    public void updateCounter()
    {
        ...
        if(counter>= debuffTime)
        {
            if (_playerIsSlow) { slowDownPlayer(); }
            if (_playerIsHeavy) { heavierPlayer(); }
            counter = 0;
        }
    }
